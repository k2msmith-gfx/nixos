...
- [ray-ecl-poc](project_ray_ecl_poc.md) — ECL scene description + Slynk (port 4005); Stage 1+2+3+4 done; 2.6× Lambert overhead (compile + type decls + single-float); tex=0.18 µs; SIGFPE footgun, Nix dev shell required
...
- [ray-janet-poc](project_ray_janet_poc.md) — Janet 1.41 scene description + TCP eval server (port 4007); Stage 1+2+3+4 done; 0.9× Lambert overhead (fiber_reset); tex=0.27 µs; pixel-perfect; comint mode + live CAPF completion; Nix dev shell required
